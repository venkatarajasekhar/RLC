...
#define Small_M 0x100000000LL  
#define Small_C 0xB16  
#define Small_A 0x5DEECE66DLL  
  
static unsigned long long seed = 1;  
  
double drand48(void)  
{  
    seed = (Small_A * seed + Small_C) & 0xFFFFFFFFFFFFLL;  
    unsigned int x = seed >> 16;  
    return  ((double)x / (double)Small_M);  
      
} 

int cmp(const void* xv, const void* yv)
{
  double const* x = (double*)xv;
  double const* y = (double*)yv;
  /* return hilbert_cmp(2, 8, 64, x, y); */
  return hilbert_ieee_cmp(3, x, y);
}
...
int main()
{
  ...
  scanf("%d", &n);
  a = (double*) malloc(dim*n*sizeof(double));
  for (i = 0; i < n; ++i)
  {
    a[3*i] = drand48()-0.5;
	a[3*i+1] = drand48()-0.5; 
  ...
  qsort(a, n, dim*sizeof(double), cmp);
